...
function normalizeSharedPayload(data) {
  ...
  const recipe = recipeRaw
    ? {
        ...recipeRaw,
        ingredients: normalizeLines(recipeRaw.ingredients),
        steps: normalizeLines(recipeRaw.steps),
      }
    : null;
  ...
  const parts = rawParts
    .map((entry) => {
      ...
      return {
        ...
        recipe: {
          ...recipePart,
          ingredients: normalizeLines(recipePart.ingredients),
          steps: normalizeLines(recipePart.steps),
        }
      };
    })
    .filter(Boolean);
...
export async function renderShareView({
  ...
  if (r?.id && isAuthenticated?.()) {
    try {
      if (await canReadRecipeAuthed(r.id)) {
        setView({ name: "detail", selectedId: r.id, q: state?.q || "" });
        return;
      }
    } catch {
      // keep public share view usable even if auth/read check fails
    }
  }
  if (!r?.id) {
    appEl.innerHTML = `
